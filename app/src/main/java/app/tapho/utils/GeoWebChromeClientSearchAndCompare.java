...
public class GeoWebChromeClientSearchAndCompare extends WebChromeClient {
    private  SearchAndCompaireFragment activity;
...
*/
    public GeoWebChromeClientSearchAndCompare(SearchAndCompaireFragment activity) {
    ...
    public void onGeolocationPermissionsShowPrompt(String origin, GeolocationPermissions.Callback callback) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M ||
                ContextCompat.checkSelfPermission(activity.getContext(), perm) == PackageManager.PERMISSION_GRANTED) {
            // we're on SDK < 23 OR user has already granted permission
            callback.invoke(origin, true, false);
        } else {
//            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, perm)) {
            // ask the user for permission
            ActivityCompat.requestPermissions(activity.getActivity(), new String[]{perm}, REQUEST_FINE_LOCATION);
