...
class TapfoFiMiniAppsAdapter<S>(private val clickListener: RecyclerClickListener) :
    ...
    inner class Holder(private val binding: TapfofiMiniappLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
        fun setData(s: S) {
            val name=""
            var image=""
            if (s is FinMiniApp){
                ...
                if (s.fin_mini_app_detail.isNullOrEmpty().not()){
                    TapfoFiNotesAdapter.addAllItem(if (s.fin_mini_app_detail.size>3) s.fin_mini_app_detail.subList(0,2) else s.fin_mini_app_detail  )
                }
