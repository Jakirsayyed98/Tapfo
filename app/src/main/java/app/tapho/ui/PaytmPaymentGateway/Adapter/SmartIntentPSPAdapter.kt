...
class SmartIntentPSPAdapter (private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<PSPModelClass, SmartIntentPSPAdapter.Holder>() {

    inner class Holder(private val binding: SmartintentpspLayoutBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun setData(s: PSPModelClass) {
    ...
    private fun unSelectAll(data: PSPModelClass) {
        list.forEach {
            if (it is PSPModelClass)
                it.isSelected = false
        }
        if (data is PSPModelClass)
            data.isSelected = true
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(
            SmartintentpspLayoutBinding.inflate(LayoutInflater.from(parent.context),
            parent,
            false))
    }
