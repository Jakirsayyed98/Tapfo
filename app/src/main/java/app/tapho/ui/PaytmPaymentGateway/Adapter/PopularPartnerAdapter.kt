...
class PopularPartnerAdapter <S>(private val clickListener: RecyclerClickListener) :
    ...
    inner class Holder(private val binding: PopularMiniappLayoutBinding) :
        ...
        fun setData(s: S) {
            ...
            if (s is Popular) {
                Glide.with(itemView.context).load(s.mini_app?.logo).centerCrop().into(binding.logo)
                binding.cashback.text =   s.cashback!!.replace("Upto","").replace("Cashback","")
                binding.root.setOnClickListener {
                    clickListener.onRecyclerItemClick(0,
                        WebViewActivity.openWebView(itemView.context, s.mini_app?.url, s.mini_app?.id, s.mini_app?.image, s.mini_app?.tcash, s.mini_app?.is_favourite, s.mini_app?.cashback, s.mini_app?.app_category_id), OPEN_WEB_VIEW)
                }
            }
        }
    }
