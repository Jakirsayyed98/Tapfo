...
class AddTagsFragment : BaseFragment<FragmentAddTagsBinding>() {
    ...
    private fun setDataInChip(data: List<Data>) {
        var chipData : ArrayList<Data> =ArrayList()
        for (name in data) {
            ...
            chip.setOnClickListener(View.OnClickListener {
                ...
                if (chip.isChecked){
                    ...
                    chip.setChipIconTintResource(R.color.green_light)
                    chip.setChipBackgroundColor(getResources().getColorStateList(R.color.Minicash))
