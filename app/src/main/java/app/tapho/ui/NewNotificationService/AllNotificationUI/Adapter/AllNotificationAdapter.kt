...
//                    image = data.data.get(0).image//.toString()
                    ...
                    if (data.merchant_postback_value.isNullOrEmpty().not()){
                        ...
                        var status = data.merchant_postback_value.get(0).status
                        if (status.uppercase()=="VERIFIED"){
                            discription = "You have recieved "+ withSuffixAmount(data.merchant_postback_value.get(0).user_commision.toString())+" Cashback from "+data.offer_name+" order id "+data.merchant_postback_value.get(0).trans_id+" on amount "+data.merchant_postback_value.get(0).sale_amount+" on "+ date+"."
                            ...
                            discription = "You have recieved "+ withSuffixAmount(data.merchant_postback_value.get(0).user_commision.toString())+" Cashback from "+data.offer_name+" order id "+data.merchant_postback_value.get(0).trans_id+" on transcation amount "+data.merchant_postback_value.get(0).sale_amount+" on "+ date + ". T&C Apply"

                        }else if (status.uppercase()=="FAILED"){
                            discription = "Your "+ withSuffixAmount(data.merchant_postback_value.get(0).user_commision.toString())+" Cashback from "+data.offer_name+" order id "+data.merchant_postback_value.get(0).trans_id+" on transcation amount "+data.merchant_postback_value.get(0).sale_amount+" on "+ date +"."
                ...
                if (image.toString().isNullOrEmpty()) {
                  //  binding.imagelayout.visibility = View.GONE
                    ...
                    Glide.with(itemView.context).load(image).placeholder(R.drawable.notification_icon).into(binding.image)
                }
