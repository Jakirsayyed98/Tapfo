...
import app.tapho.databinding.FragmentEnterAmountForBillBinding
...
class EnterAmountForBillFragment : BaseFragment<FragmentEnterAmountForBillBinding>() {
    var ServiceTypeID = ""
    var Name = ""
    var icon = ""
    var CustomerIDType = ""
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEnterAmountForBillBinding.inflate(inflater, container, false)
        statusBarTextWhite()
    ...
    private fun setAllText() {
        ...
        var startRange = AmountRange.replaceAfter("-", "")
        var endRange = AmountRange.removeRange(0, 4)
        _binding!!.btnVerify.setOnClickListener {
            if (_binding!!.Amount.text.toString().isNullOrEmpty().not()) {
                if (_binding!!.Amount.text.toString().toDouble() >= startRange.replace("-", "")
                        .toString().toDouble()
                ) {
                    if (_binding!!.Amount.text.toString().toDouble() < endRange.replace("-", "").toString().toDouble()) {
                        ...
                        Snackbar.make(
                            requireView(),
                            "Please enter amount within $startRange to $endRange ",
...
//                        _binding!!.Amount.setText(totalAmount.toString())
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance() =
            EnterAmountForBillFragment().apply {
