...
//                _binding!!.timer.setText("0" + minute.toString() + ":" + seconds.toString()).toString()
        ...
        viewModel.getTransactionStatus(getUserId(), OrderId, this,
            object : ApiListener<TransactionStatusRes, Any?> {
                override fun onSuccess(t: TransactionStatusRes?, mess: String?) {
                    t.let {
                        it!!.body.let {
                            var result_status = it.resultInfo.resultStatus
                            if (result_status == "TXN_SUCCESS") {
                                ...
                                coundown.cancel()
                                ContainerActivity.openContainerforPaymentStatus(requireContext(),"AddMoneyTransactionStatus",it.resultInfo.resultCode,it.txnId,it.resultInfo.resultMsg,"Paytm",pspnamedata)
                           //     PaymentFailedTimeOut()
                                activity?.finish()
                            }
                        }
    ...
    private fun ProceedToAddMoneyApiCall(txnAmount: String,payOption : String) {
        ...
        var randomString = List(7) { charset.random() }.joinToString("")
...
//        Glide.with(requireContext()).asGif().load(R.drawable.loading_progress).circleCrop().into(_binding!!.rechargeStatus)
        ...
        viewModel.AddMoneyToWallet(getUserId(),txnAmount,"0",txn_id,payoptionName.toString(),this,object : ApiListener<AddMoneyRes,Any?>{
            override fun onSuccess(t: AddMoneyRes?, mess: String?) {
                t.let {
                    ...
                    if (t!!.errorCode.toString().equals("0")){
                        ...
                        setToBlankData()
                        ContainerActivity.openContainerforPaymentStatus(requireContext(),"AddMoneyTransactionStatus",it.errorCode,txn_id,it.errorMsg,"wallet",pspnamedata)
                    }else{
                        setToBlankData()
                        ContainerActivity.openContainerforPaymentStatus(requireContext(),"AddMoneyTransactionStatus",it!!.errorCode,txn_id,it.errorMsg,"wallet",pspnamedata)
