...
class BannerAdapter : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        Glide.with(view).load(list[position].image).apply(RequestOptions().transform(RoundedCorners(40))).into(view.findViewById(R.id.image))

        view.setOnClickListener {
            if (list[position].url!!.contains("https")){
                WebViewActivity.openWebView(view.context,list[position].url,"","","1","","","")
            }
            else
            {
