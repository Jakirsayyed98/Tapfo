...
import app.tapho.R
import app.tapho.databinding.RowMerchantDealsAndofferBinding
...
class CustomeOfferListAdapter (private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<OfferData, CustomeOfferListAdapter.Holder>() {

    inner class Holder(val binding: RowMerchantDealsAndofferBinding) :
...
*/
            ...
            if (data.type == "2") {
                ...
                binding.offerandCoupons.text = "Offers"
            } else {

                binding.offerandCoupons.text = "Coupon"
            }

            binding.nameTv.text=data.mini_app.get(0).name
            binding.descriptionTv.text ="Get"+data.label!!.replace("Upto"," ")+data.name
            binding.visitedTv.text = itemView.context.getString(R.string._interested, data.visited)
            binding.visited.text = itemView.context.getString(R.string._7_people_used_today, data.visited)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(
            RowMerchantDealsAndofferBinding.inflate(
