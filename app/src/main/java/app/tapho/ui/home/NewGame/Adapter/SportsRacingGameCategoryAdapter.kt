...
class SportsRacingGameCategoryAdapter<S>(private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<S, SportsRacingGameCategoryAdapter<S>.Holder>()  {
    var totalplays=""
...
//    inner class Holder(private val binding: RowGamesBinding) :
    inner class Holder(private val binding: RecenPlayedGameLayoutBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun setData(s: S) {

            if (s is Data) {
                Glide.with(itemView.context).load(s.assets.square).circleCrop().centerCrop().placeholder(
                    R.drawable.loding_app_icon).into(binding.ivPartner)
