...
import app.tapho.R
import app.tapho.databinding.FavMerchantlayoutBinding
...
class HomeFavAdapter (private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<MiniApp, HomeFavAdapter.Holder>() {
...
//            }
    ...
    inner class Holder(private val binding: FavMerchantlayoutBinding) :
...
//                binding.rupee.visibility = View.INVISIBLE
                when (s.name) {
                    itemView.context.getString(R.string.more) -> {
                        Glide.with(itemView.context).load(R.drawable.ic_more_blue).placeholder(R.drawable.loding_app_icon).into(binding.ivPartner)
...
//                    ActiveCashbackForWebActivity.openWebView(it.context,url,miniAppId,image,tCashType,isFavourite)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(
            FavMerchantlayoutBinding.inflate(
