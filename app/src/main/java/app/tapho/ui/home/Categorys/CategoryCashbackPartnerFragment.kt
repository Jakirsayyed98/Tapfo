...
class CategoryCashbackPartnerFragment : BaseFragment<FragmentCategoryCashbackPartnerBinding>(),
    ...
    private fun getData() {
        ...
        activity?.intent?.getStringExtra(DATA)?.let {
            if (oldData == false) {
                ...
                viewModel.getAppCategoryMiniAppwithBanner(
                    AppSharedPreference.getInstance(requireContext()).getUserId(),
                    category.id, this, this
                )
            } else {
    ...
    override fun onRecyclerItemClick(pos: Int, data: Any?, type: String) {
        if (type == OPEN_WEB_VIEW) {
            if (data is MiniApp)
                WebViewActivity.openWebView(context, data.url, data.id, data.image, data.tcash.toString(), data.is_favourite.toString(), data.cashback,
