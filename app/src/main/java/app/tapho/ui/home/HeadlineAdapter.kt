package app.tapho.ui.home

import android.view.LayoutInflater
import android.view.ViewGroup
import app.tapho.databinding.RowHeadlineBinding
import app.tapho.interfaces.RecyclerClickListener
import app.tapho.ui.BaseRecyclerAdapter
import app.tapho.ui.home.adapter.HeadlineHolder
import app.tapho.ui.model.AppCategory
import app.tapho.ui.model.NewHomeRes.Headline
import app.tapho.ui.model.NewHomeRes.HomeResData

class HeadlineAdapter (private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<Headline/*AppCategory*/, HeadlineHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HeadlineHolder {
        return HeadlineHolder(RowHeadlineBinding.inflate(LayoutInflater.from(parent.context),
            parent, false), clickListener)
    }

    override fun onBindViewHolder(holder: HeadlineHolder, position: Int) {
        holder.setData(list[position])
    }

    override fun getItemCount(): Int {
        return list.size
    }

}