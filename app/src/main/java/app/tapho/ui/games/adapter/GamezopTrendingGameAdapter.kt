...
import app.tapho.R
import app.tapho.databinding.FeturedPartnerLayoutBinding
...
class GamezopTrendingGameAdapter<S>(private val clickListener: RecyclerClickListener) :
    BaseRecyclerAdapter<S, GamezopTrendingGameAdapter<S>.Holder>()  {
    var totalplays=""
    ...
    inner class Holder(private val binding: FeturedPartnerLayoutBinding) :
...
//                val myRandomValues =  List(10) { Random.nextInt(0, listSize) }
                ...
                Glide.with(itemView.context).load(s.assets.square).placeholder(R.drawable.loding_app_icon).centerCrop().into(binding.ivPartner)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(
            FeturedPartnerLayoutBinding.inflate(
