...
import app.tapho.ui.merchants.adapter.NewCategoryBannerAdapter
    ...
    private fun getData() {
        ...
        activity?.intent?.getStringExtra(DATA)?.let {
            if (oldData == false) {
                ...
                Glide.with(requireContext()).load(category.image).circleCrop().into(bindingNewCat!!.iconCard)
                bindingNewCat!!.Title.text=activity?.intent?.getStringExtra(TITLE)
                viewModel.getAppCategoryMiniApp(AppSharedPreference.getInstance(requireContext()).getUserId(), category.id, this, this)
            } else {
