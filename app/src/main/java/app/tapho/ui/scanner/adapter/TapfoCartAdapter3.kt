...
class TapfoCartAdapter3<S>(private val clickListener: RecyclerClickListener) :
    ...
    inner class Holder(private val binding: Tapfocart3LayoutBinding) :
        ...
        fun setData(s: S) {
            if (s is Item) {
                s.business_user_item_detail.let {
                    ...
                    binding.price.text =
                        withSuffixAmount(it.mrp)!!.dropLast(3) + " ( " + s.qty + " )"
                    ...
                    binding.savepercent.visibility =
                        if (s.price.isNullOrEmpty().not()) View.VISIBLE else View.GONE
