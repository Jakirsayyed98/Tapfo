...
class MiniCashMiniAppListFragment : BaseFragment<FragmentMiniCashMiniAppListBinding>(),RecyclerClickListener {
    ...
    private var miniAppRes: MiniAppRes1? = null
...
//        )
    ...
    private fun getCategoryData(AppCategoryID: String) {
        ...
        viewModel.getAppCategoryMiniAppwithBanner(getUserId(), categoryID, this, object : ApiListener<MiniAppRes1, Any?> {
...
//            ActiveCashbackActivity.openWebView(context, data.url, data.id, data.image, data.tcash, data.is_favourite, data.cashback, data.app_category_id)
        WebViewActivity.openWebView(context, data.url, data.id, data.image, data.tcash.toString(), data.is_favourite.toString(), data.cashback, data.app_category_id)
    }
}